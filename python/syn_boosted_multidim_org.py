...
domainclouds = dict()
domaintags = dict()
tagdomsimfile = '/home/fnargesian/go/src/github.com/RJMillerLab/opendata-ontology/python/synthetic_output/tag_domain_sims.json'
simfile = '/home/fnargesian/go/src/github.com/RJMillerLab/opendata-ontology/python/synthetic_output/allpair_sims.json'
tagdomtransprobsfile = '/home/fnargesian/go/src/github.com/RJMillerLab/opendata-ontology/python/synthetic_output/tag_dom_trans_probs.json'
...
def init(tag_num):
    ...
    # finding similar tags to a tag
    simtags = get_topk_tags_to_tag(keys, vecs)
    sumprev = 0.0
    sumnew = 0.0
    for dom, dtags in domaintags.items():
        sumprev += len(dtags)
        added = 0
        for dt in dtags:
            for at in simtags[dt]:
                if added >= 3:
                    continue
                if at != dt and at not in domaintags[dom]:
                    added += 1
                    domaintags[dom].append(at)
                    tagdomains[at].append(dom)
        sumnew += len(domaintags[dom])
    print('boosted tags')
...
def get_topk_tags_to_tag(tags, vecs):
    tsims = dict()
    for i in range(len(tags)):
        if tags[i] not in tsims:
            tsims[tags[i]] = dict()
        for j in range(i+1,len(tags)):
            s = orgh.cosine(vecs[i], vecs[j])
            if tags[j] not in tsims:
                tsims[tags[j]] = dict()
            if tags[i] not in tsims:
                tsims[tags[i]] = dict()
            tsims[tags[i]][tags[j]] = s
            tsims[tags[j]][tags[i]] = s

    simtags = dict()
    for q, tss in tsims.items():
        ssims = sorted(tss.items(), key=operator.itemgetter(1), reverse=True)
        simtags[q] = []
        for s in ssims:
            if s == q:
                continue
            simtags[q].append(s[0])
    return simtags
...
def fix(g, hierarchy_name):
    print('fix')
    h, stats, iteration_ls, sps, dsps = orgf.fix_plus(g, domains, tagdomains, domainclouds, 'synthetic', domaintags)

    print('sps from fix_plus is %f' % (sum(list(sps.values()))/len(sps)))

    #print('fuzzy eval: ')
    #max_success, gp, success_probs, likelihood, domain_success_probs = orgh.get_success_prob_fuzzy(h.copy(), domains, tagdomains, domainclouds, 'synthetic', domaintags)
    success_probs = sps
    domain_success_probs = dsps
...
def agg_fuzzy(suffix1, suffix2):
    ...
    orgh.init(gp, domains, simfile, tagdomtransprobsfile, True)
    max_success, gp, success_probs, likelihood, domain_success_probs = orgh.get_success_prob_fuzzy(gp.copy(), domains, tagdomains, domainclouds, 'synthetic', domaintags)
...
def multidimensional_hierarchy(dim_num):
    ...
    for i, ts in dims.items():
        ...
        gp, sps, before_dsps = agg_fuzzy('agg'+str(i)+'f2op_boosted_with3', '')
        ...
        sps, after_dsps, dim_stats = fix(gp, 'agg_'+str(i)+'f2op_boosted_with3_')
    ...
    multi_json = '/home/fnargesian/go/src/github.com/RJMillerLab/opendata-ontology/python/synthetic_output/multidim_dists_boosted_before_' + str(len(alldomains)) + '_' + str(dim_num) + '.json'
    json.dump(success_probs_before, open(multi_json, 'w'))
    print('printed mult before results to %s.' % multi_json)

    multi_dom_json = '/home/fnargesian/go/src/github.com/RJMillerLab/opendata-ontology/python/synthetic_output/multidim_domain_probs_boosted_before_' + str(len(alldomains)) + '_' + str(dim_num) + '_g10rhap.json'
    json.dump(before_dsps, open(multi_dom_json, 'w'))
    print('printed domain mult results before to %s.' % multi_dom_json)

    multi_json = '/home/fnargesian/go/src/github.com/RJMillerLab/opendata-ontology/python/synthetic_output/multidim_dists_boosted_' + str(len(alldomains)) + '_' + str(dim_num) + '_g10rhap.json'
    json.dump(success_probs_after, open(multi_json, 'w'))
    print('printed mult after results to %s.' % multi_json)

    multi_dom_json = '/home/fnargesian/go/src/github.com/RJMillerLab/opendata-ontology/python/synthetic_output/multidim_domain_probs_boosted_after_' + str(len(alldomains)) + '_' + str(dim_num) + '_g10rhap.json'
    json.dump(after_dsps, open(multi_dom_json, 'w'))
    print('printed domain mult results after fix to %s.' % multi_dom_json)

    json.dump(stats, open('synthetic_output/fix_' + str(len(domains)) + '_boosted.json',    'w'))
    print('printed stats to %s' % ('synthetic_output/fix_' + str(len(domains)) + '_boosted.json'))
